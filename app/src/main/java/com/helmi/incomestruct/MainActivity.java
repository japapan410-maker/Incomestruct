...
public class BerandaFragment extends Fragment {
    ...
    private TextView tvDailyAmount, tvGridPendapatan, tvGridPengeluaran, tvGridTabungan, tvGridDarurat;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        // Jalankan pengecekan reset harian otomatis
    ...
    private void checkAndResetDailyData() {
        ...
        // Ambil tanggal hari ini (format: TahunBulanTanggal, cth: 20260611)
        String currentDate = new SimpleDateFormat("yyyyMMdd", Locale.getDefault()).format(new Date());
        String lastResetDate = prefs.getString("last_reset_date", "");

        // Jika hari sudah berganti, reset nilai harian ke 0
    ...
    private void loadDailyData() {
        SharedPreferences prefs = requireActivity().getSharedPreferences(PREF_NAME, Context.MODE_PRIVATE);
        float totalIncome = prefs.getFloat("daily_pendapatan", 0f);
        float totalExpense = prefs.getFloat("daily_pengeluaran", 0f);
        float totalTabungan = prefs.getFloat("daily_tabungan", 0f);
        float totalDarurat = prefs.getFloat("daily_darurat", 0f);

        float netDaily = totalIncome - totalExpense;

        tvDailyAmount.setText("Rp" + (int) netDaily);
        tvGridPendapatan.setText("Rp" + (int) totalIncome);
        tvGridPengeluaran.setText("Rp" + (int) totalExpense);
        tvGridTabungan.setText("Rp" + (int) totalTabungan);
        tvGridDarurat.setText("Rp" + (int) totalDarurat);
    }

    private void saveTransaction(float nominal) {
        SharedPreferences prefs = requireActivity().getSharedPreferences(PREF_NAME, Context.MODE_PRIVATE);
        float currentIncome = prefs.getFloat("daily_pendapatan", 0f) + nominal;

        SharedPreferences.Editor editor = prefs.edit();
        editor.putFloat("daily_pendapatan", currentIncome);
        editor.apply();

        etNominal.setText("");
        loadDailyData();
        Toast.makeText(getContext(), "Cashflow harian berhasil dicatat!", Toast.LENGTH_SHORT).show();
    }
}
