...
public class MainActivity extends Activity {
    ...
    private void renderBeranda() {
        ...
        // Hitung khusus Cashflow Harian (Hanya hari ini)
        long cashflowHarian = 0;
        ...
        if (!logs.isEmpty()) {
            ...
            for (String item : items) {
                if (item.contains(tanggalHariIni)) {
                    try {
                        String rawNom = item.split("—")[0].replaceAll("\\[\\+\\]|\\[\\-\\]", "").trim();
                        long val = Long.parseLong(rawNom);
                        if (item.contains("[+]")) {
                            cashflowHarian += val;
                        } else {
                            cashflowHarian -= val;
        ...
        tvSaldo.setText(formatRupiah(cashflowHarian));
